  ...
  const PoseWithRange & pose_with_range, const Shape & shape) const
{
  ...
  const auto pose_min_polygon = autoware_utils::to_polygon2d(pose_with_range.pose_min, shape);
  const auto pose_max_polygon = autoware_utils::to_polygon2d(pose_with_range.pose_max, shape);
  ...
  std::vector<geometry_msgs::msg::Point> & collision_points) const
{
  ...
  for (const auto & point : bg_ranged_polygon.outer()) {
    const auto p_msg =
      autoware_utils::create_point(point.x(), point.y(), pose_with_range.pose_min.position.z);
