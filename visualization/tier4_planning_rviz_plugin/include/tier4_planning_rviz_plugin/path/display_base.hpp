...
protected:
  ...
  void visualizePathFootprint(const typename T::ConstSharedPtr msg_ptr)
  {
    ...
    const size_t point_triangles = property_point_view_.getBool() ? 8 * 3 : 0;  // 8 triangles per circle
    const size_t pose_triangles = property_pose_view_.getBool() ? 3 : 0;  // 3 triangles per arrow
    point_manual_object_->estimateVertexCount(msg_ptr->points.size() * (point_triangles + pose_triangles));
